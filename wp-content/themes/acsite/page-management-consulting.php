...
<div id="wrap-management-consulting">
	<?php if (have_posts()) : while (have_posts()) : the_post(); ?>
		...
		<?php $management_consulting_header = get_post_meta( $post->ID, 'management_consulting_header', true ); ?>
		<div class="service-title container">
	        ...
	        <div class="div-title">
                <?php if(!empty($management_consulting_header[0]['title'])) { ?>
					<h1><?php echo $management_consulting_header[0]['title'];?></h1>
				<?php } ?>

				<hr class="border-title"></hr>

				<?php if(!empty($management_consulting_header[0]['subtitle'])) { ?>
					<h2><?php echo $management_consulting_header[0]['subtitle'];?></h1>
				<?php } ?>
	        </div>
        </div>

        <?php $management_consulting_items = get_post_meta( $post->ID, 'management_consulting_body', true ); ?>
		<?php if(!empty($management_consulting_items)) { ?>
	        <div class="service-onpage-menu container">
	        	<ul class="text-center">
	                <?php 
	                	foreach( $management_consulting_items as $management_consulting_item) {
